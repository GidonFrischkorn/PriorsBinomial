...
│   ├── 01_PriorPredictive.R        # Goal 1: SimDesign prior predictive (fixed effects)
│   ├── 01b_PriorPredictive_RE.R    # Goal 1: Prior predictive with random effects
│   ├── 02_SaveData_PriorPredictive.R       # Goal 1: post-process fixed-effects results
...
│   ├── 04_SaveData_BFCalibration.R         # Goal 2: post-process fixed-effects BF results
│   └── 04b_SaveData_BFCalibration_RE.R     # Goal 2: post-process RE BF results
│
├── reports/                        # Quarto documents
│   ├── prior_predictive_analysis.qmd   # Main report (HTML / PDF / DOCX)
│   ├── bf_calibration_analysis.qmd     # BF calibration report
│   └── references.bib              # BibTeX bibliography
│
├── output/                         # Simulation outputs (gitignored)
│   ├── res_prior_predictive.rds
│   ├── prior_predictive_summaries.rds
│   ├── prior_predictive_draws.rds
│   ├── res_bf_calibration.rds
│   └── Simulation_PriorPredictive/   # Per-condition .rds files
│
...
```r
quarto::quarto_render("reports/prior_predictive_analysis.qmd")
```

Or from the terminal:

```bash
quarto render reports/prior_predictive_analysis.qmd
```

The report renders to HTML, PDF, and DOCX simultaneously.
...
Ready-to-use `brms` prior specifications are provided in the report's Recommendations section.
